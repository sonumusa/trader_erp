...
$val = fn(string $k, string $col = '') => $this->e($old[$k] ?? ($customer[$col ?: $k] ?? ''));
?>
<?php $this->layout('app'); ?>
    ...
    <div>
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><?= $customer ? 'Edit' : 'New' ?></li>
            </ol>
        </nav>
    </div>
</div>
    ...
    <div class="col-lg-9">
        <div class="erp-card">
            <div class="erp-card-body">
                <form method="post" action="<?= $customer ? $this->url('/customers/' . (int) $customer['id']) : $this->url('/customers') ?>">
                    <?= $this->csrfField() ?>

                    <div class="row g-3">
                        <div class="col-md-8">
                            <label class="form-label" for="name">Customer name <span class="required-star">*</span></label>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select" id="type" name="type">
                                <option value="business" <?= ($customer['type'] ?? 'business') === 'business' ? 'selected' : '' ?>>Business</option>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" step="0.01" class="form-control" id="credit_limit" name="credit_limit" value="<?= $val('credit_limit', 'credit_limit') ?>">
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-hint">Positive = customer owes you. Posted to Accounts Receivable with the opening equity entry.</div>
                        </div>
                        <?php endif; ?>
                    ...
                    <div class="mt-4 d-flex gap-2">
                        <button type="submit" class="btn btn-primary btn-icon">
                            <i class="bi bi-check-lg"></i> <?= $customer ? 'Save Changes' : 'Create Customer' ?>
                        </button>
                        <a href="<?= $this->url('/customers') ?>" class="btn btn-outline-secondary">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="col-lg-3">
        <div class="erp-card">
            ...
            <div class="erp-card-body small">
                <div class="d-flex justify-content-between"><span class="text-muted">Code</span><b><?= $this->e($customer['code']) ?></b></div>
                <div class="d-flex justify-content-between mt-1"><span class="text-muted">Opening balance</span><b><?= $this->e(format_money($customer['opening_balance'])) ?></b></div>
                <hr class="my-2">
                <a href="<?= $this->url('/customers/' . (int) $customer['id'] . '/ledger') ?>" class="btn btn-sm btn-outline-primary w-100 btn-icon">
