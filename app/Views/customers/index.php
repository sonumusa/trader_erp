...
<div class="erp-page-head">
    ...
    <div class="actions">
        <?php if ($this->data['canCreate']): ?>
            <a href="<?= $this->url('/customers/new') ?>" class="btn btn-primary btn-icon">
...
<div class="erp-card">
    <div class="erp-card-body">
        <form method="get" action="<?= $this->url('/customers') ?>" class="row g-2 align-items-center mb-2">
            ...
            <table class="table erp-table">
                ...
                <?php foreach ($this->data['rows'] as $c): ?>
                    <tr>
                        ...
                        <td><?= $this->e($c['city'] ?: '—') ?></td>
                        <td><span class="badge badge-soft-neutral"><?= $this->e(ucfirst($c['type'])) ?></span></td>
                        <td class="num">
                        ...
                        <td class="actions-cell">
                            <a href="<?= $this->url('/customers/' . (int) $c['id'] . '/ledger') ?>" class="btn btn-sm btn-outline-primary btn-icon" title="Ledger">
                                <i class="bi bi-journal-text"></i>
                            </a>
                            <?php if ($this->data['canEdit']): ?>
                                <a href="<?= $this->url('/customers/' . (int) $c['id'] . '/edit') ?>" class="btn btn-sm btn-outline-secondary btn-icon" title="Edit">
                ...
                <?php if (!$this->data['rows']): ?>
                    <tr><td colspan="7"><div class="empty-state"><i class="bi bi-people"></i>No customers found.</div></td></tr>
                <?php endif; ?>
                </tbody>
            </table>
        </div>
            ...
            <nav><ul class="pagination">
                <?php for ($i = 1; $i <= $this->data['pages']; $i++): ?>
                    <li class="page-item <?= $i === $this->data['page'] ? 'active' : '' ?>">
                        <a class="page-link" href="<?= $this->url('/customers?page=' . $i . '&q=' . urlencode($this->data['search'])) ?>"><?= $i ?></a>
                    </li>
                <?php endfor; ?>
            </ul></nav>
        <?php endif; ?>
