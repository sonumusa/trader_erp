...
<div class="erp-page-head">
    <div>
        <h1>Supplier Ledger — <?= $this->e($supplier['name']) ?></h1>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="<?= $this->url('/') ?>">Home</a></li>
                <li class="breadcrumb-item"><a href="<?= $this->url('/suppliers') ?>">Suppliers</a></li>
                <li class="breadcrumb-item active"><?= $this->e($supplier['code']) ?></li>
            </ol>
        </nav>
    </div>
    <div class="actions">
        ...
        <a href="<?= $this->url('/suppliers/' . (int) $supplier['id'] . '/edit') ?>" class="btn btn-outline-primary btn-icon">
    ...
    <div class="col-lg-8">
        <div class="erp-card h-100">
            ...
            <div class="erp-card-body">
                <form method="get" action="<?= $this->url('/suppliers/' . (int) $supplier['id'] . '/ledger') ?>" class="row g-2 align-items-end">
            ...
            <nav class="p-2"><ul class="pagination">
                <?php for ($i = 1; $i <= $data['pages']; $i++): ?>
                    <li class="page-item <?= $i === $data['page'] ? 'active' : '' ?>">
                        <a class="page-link" href="<?= $this->url('/suppliers/' . (int) $supplier['id'] . '/ledger?page=' . $i . '&from=' . urlencode($from) . '&to=' . urlencode($to)) ?>"><?= $i ?></a>
                    </li>
                <?php endfor; ?>
            </ul></nav>
        <?php endif; ?>
